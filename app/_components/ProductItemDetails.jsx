...
//           {product.attributes.selling_price &&
//             <h2 className='font-bold text-3xl'>${product.attributes.selling_price}</h2>
//           }
//           <h2 className={`font-bold text-3xl ${product.attributes.selling_price && 'line-through text-gray-500'}`}>
//             ${product.attributes.mrp}
//           </h2>
//         </div>
...
//           </div>
//           <Button className='flex px-5 gap-6 mt-4 p-2' onClick={()=>addToCart()}
//             disabled={loading}
//             >
//             <ShoppingBasket />
...
//           </Button>
//         </div>
//         {product.attributes.categories && product.attributes.categories.data && product.attributes.categories.data.length > 0 ? (
//           <h2><span>Category:</span> {product.attributes.categories.data[0].attributes.name}</h2>
//         ) : (
//           <h2><span>Category:</span> No category available</h2>
//         )}
//       </div>
//     </div>
//   );
...
function ProductItemDetails({ product }) {
  ...
  const { updateCart, setUpdateCart } = useContext(UpdateCartContext);
  const [productTotalPrice, setProductTotalPrice] = useState(
    product.attributes.selling_price ? product.attributes.selling_price : product.attributes.mrp
  );
  const router = useRouter();
  const [quantity, setQuantity] = useState(1);
  const [loading, setLoading] = useState(false);
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 p-7 bg-white text-black'>
      ...
      <div className='flex flex-col gap-3'>
        ...
        <div className='flex gap-3'>
          ...
          <h2
            className={`font-bold text-3xl ${
              product.attributes.selling_price && 'line-through text-gray-500'
            }`}
          >
            ${product.attributes.mrp}
          </h2>
        ...
        <div className='flex flex-col items-baseline gap-3'>
          ...
          <Button className='flex px-5 gap-6 mt-4 p-2' onClick={() => addToCart()} disabled={loading}>
            <ShoppingBasket />
            {loading ? <LoaderCircle className='animate-spin' /> : 'Add To Cart'}
          </Button>
        </div>
